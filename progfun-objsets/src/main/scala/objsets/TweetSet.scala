...
abstract class TweetSet {
  ...
  def mostRetweetedAcc(currentMostRetweeted: Tweet): Tweet;
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    new NonEmpty(elem, left.union(that.filter(tw => tw.text < elem.text)), right.union(that.filter(tw => tw.text > elem.text))) 
  }
  ...
  def mostRetweetedAcc(currentMostRetweeted: Tweet): Tweet = {
    if (elem.retweets > currentMostRetweeted.retweets) {
      ...
      right.mostRetweetedAcc(left.mostRetweetedAcc(currentMostRetweeted))
    }
  }
